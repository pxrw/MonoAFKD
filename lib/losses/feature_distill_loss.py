...
def compute_backbone_local_affinity_loss(features_preds, features_targets):
    ...
    if isinstance(features_preds, list):
        for i in range(len(features_preds)):  # 1/8   1/16   1/32
            ...
            feature_pred_q = rearrange(feature_pred, 'b c (h p1) (w p2) -> (b h w) (p1 p2) c', p1=local_shape[0],
                                       p2=local_shape[1])
            feature_pred_k = rearrange(feature_pred, 'b c (h p1) (w p2) -> (b h w) (p1 p2) c', p1=local_shape[0],
                                       p2=local_shape[1])

            rgb_affinity = einsum('b i d, b j d -> b i j', feature_pred_q, feature_pred_k)

            feature_target_q = rearrange(feature_target, 'b c (h p1) (w p2) -> (b h w) (p1 p2) c', p1=local_shape[0],
                                         p2=local_shape[1])
            feature_target_k = rearrange(feature_target, 'b c (h p1) (w p2) -> (b h w) (p1 p2) c', p1=local_shape[0],
                                         p2=local_shape[1])
...
########################
def compute_dlaup_loss(features_preds, features_targets, target):
    feature_ditill_loss = 0.0
    if isinstance(features_preds, list):
        for i in range(len(features_preds)):
            downsample_ratio = 2 ** (i + 2)
