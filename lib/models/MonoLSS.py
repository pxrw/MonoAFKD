...
class MonoLSS(nn.Module):
    def __init__(self, backbone='dla34', neck='DLAUp', downsample=4, mean_size=None, model_type='test'):
...
if __name__ == '__main__':
    ...
    mean_size = dataset.cls_mean_size
    dataloader = DataLoader(dataset=dataset, batch_size=1)
    data_sample = next(iter(dataloader))
    inputs, calibs, coord_range, targets, info = data_sample
    image = inputs['rgb']
    depth = inputs['depth']
    print(f'image shape: {image.shape}, depth shape: {depth.shape}')

    m = MonoLSS(mean_size=mean_size, model_type='monolss').cuda(0)
    for key in inputs.keys():
        inputs[key] = inputs[key].cuda(0)

    calibs = calibs.cuda(0)
    coord_range = coord_range.cuda(0)

    for key in targets.keys():
        targets[key] = targets[key].cuda(0)

    outs = m(inputs, coord_range, calibs, targets)
