...
class Trainer(object):
    def __init__(self,
        ...
        if self.model_type != 'distill':
            if self.cfg_train.get('pretrain_model'):
                ...
                load_checkpoint(model=self.model, optimizer=None, filename=self.cfg_train['pretrain_model'],
            ...
            if self.cfg_train.get('pretrain_model'):
                if os.path.exists(self.cfg_train['pretrain_model']['rgb']):
                    load_checkpoint(model=self.model.centernet_rgb, optimizer=None,
                                    filename=self.cfg_train['pretrain_model']['rgb'], map_location=self.device,
                                    logger=self.logger)
                else:
                ...
                if os.path.exists(self.cfg_train['pretrain_model']['depth']):
                    load_checkpoint(model=self.model.centernet_depth, optimizer=None,
                                    filename=self.cfg_train['pretrain_model']['depth'], map_location=self.device,
                                    logger=self.logger)
                else:
            ...
            if self.cfg_train.get('resume_model', None):
                assert os.path.exists(self.cfg_train['resume_model'])
                self.epoch = load_checkpoint(model=self.model, optimizer=self.optimizer,
                                             filename=self.cfg_train['resume_model'], map_location=self.device,
                                             logger=self.logger)
                self.lr_scheduler.last_epoch = self.epoch - 1
        ...
        with torch.no_grad():
            for batch_idx, (inputs, calibs, coord_ranges, targets, info) in enumerate(self.train_loader):
                ...
                if self.model_type != 'distill':
                    ...
                    rgb_outputs, backbone_loss_affinity, head_loss, align_freq_loss = self.model(inputs, coord_ranges,
                                                                                                 calibs, targets,
                                                                                                 self.epoch)
                    criterion = LSS_Loss(self.epoch)
    ...
    def train_one_epoch_distill(self, loss_weights=None):
        ...
        if 'align_freq_kd' in self.kd_type:
            loss_stats.append('align_freq_kd')
        ...
        for batch_idx, (inputs, calibs, coord_ranges, targets, info) in enumerate(self.train_loader):
            ...
            rgb_outputs, backbone_loss_affinity, head_loss, align_freq_loss = self.model(inputs, coord_ranges, calibs, targets)
            ...
            if 'align_freq_kd' in self.kd_type:
                align_freq_loss = (align_freq_loss * 10).mean()
                if 'align_freq_kd' not in stat_dict.keys():
                    stat_dict['align_freq_kd'] = 0
                stat_dict['align_freq_kd'] += align_freq_loss

                if 'align_freq_kd' not in disp_dict.keys():
                    disp_dict['align_freq_kd'] = 0
                disp_dict['align_freq_kd'] += align_freq_loss

                total_loss = total_loss + align_freq_loss
