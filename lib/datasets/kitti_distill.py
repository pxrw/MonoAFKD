...
class KITTI(data.Dataset):
    def __init__(self, root_dir, split, cfg):
        ...
        # path configuration
        self.data_dir = os.path.join(root_dir, cfg['data_dir'], 'kitti', 'testing' if split == 'test' else 'training')
    ...
    def __getitem__(self, item):
        ...
        if self.split != 'test':
            d = cv.imread('{}/{:0>6}.png'.format(self.dense_depth_dir, index), -1) / 256.0
        ...
        if random_mix_flag == True:
            ...
            while count_num < 50:
                ...
                if calib_temp.cu == calib.cu and calib_temp.cv == calib.cv and calib_temp.fu == calib.fu and calib_temp.fv == calib.fv:
                    ...
                    if self.split != 'test' and (dst_W == dst_W_temp and dst_H == dst_H_temp):
                        d_temp = cv.imread('{}/{:0>6}.png'.format(self.dense_depth_dir, random_index), -1) / 256.0
                        pad_h, pad_w = dst_H - d_temp.shape[0], (dst_W - d_temp.shape[1]) // 2
                        pad_wr = dst_W - pad_w - d_temp.shape[1]
                        ...
                        if len(objects_1) + len(objects_2) < self.max_objs:
                            ...
                            img_temp = np.array(img_temp).astype('uint8')
                            img_blend = np.zeros_like(img)
                            img_blend[self.mask] = img[self.mask]
                            img_blend[~self.mask] = img_temp[~self.mask]
                            img = img_blend
                            ...
                            if self.split != 'test':
                                ...
                                d_temp = np.array(d_temp).astype(np.float32)
                                d_blend = np.zeros_like(d)
                                d_blend[self.mask] = d[self.mask]
                                d_blend[~self.mask] = d_temp[~self.mask]
                                d = d_blend
                                d = Image.fromarray(d)
                            break
        ...
        if self.split != 'test':
            depth_map = np.array(d_trans).astype(np.float32) / 256.0
        ...
        if self.split != 'test':
            ...
            if random_mix_flag == True:
                ...
                for i in range(object_num_temp):
                    ...
                    box2d_gt[i + object_num, :] = bbox_2d
                    box2d_gt_head[i + object_num, :] = bbox_2d
            ...
            targets = {'depth': depth,
                       ...
                       'obj_num': object_num + object_num_temp,
                       'box2d_gt': box2d_gt,
...
if __name__ == '__main__':
    ...
    dataset = KITTI('/home/pxr/pxrProject/3Ddetection/MonoLSS', 'train', cfg)
    dataloader = DataLoader(dataset=dataset, batch_size=1)
    print(dataset.writelist)

    data_sample = next(iter(dataloader))
    image = data_sample[0]['rgb']
    depth = data_sample[0]['depth']
    # targets = data_sample[3]

    print(f'image shape: {image.shape}, depth shape: {depth.shape}')
    print(depth[0][..., 250, 800])
