...
def get_bev_image(pc_velo, objects, calib):
    side_range = (-30, 30)  # 左右距离
    fwd_range = (0, 80)  # 后前距离
...
class kitti_object(object):
    ...
    def __init__(self, image_path, label_path, calib_path, lidar_path):
    ...
    def get_image(self):
        return utils.load_image(self.image_path)

    def get_lidar(self):
        return utils.load_velo_scan(self.lidar_path)

    def get_calibration(self):
        return utils.Calibration(self.calib_path)

    def get_label_objects(self):
        return utils.read_label(self.label_path)
